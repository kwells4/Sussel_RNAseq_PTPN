...
We start by providing a path to the location of the count table. This count table was created using a combination of STAR for aligning and feature counts for gene counting. I like to provide a "base path" that is where all documents associated with this analysis are located. That way, I make sure that I always am saving to the correct place even if I move the analysis (I only have to change the base path and not every line of code).

```{r}
base_path <- "/Users/wellskr/Documents/Analysis/Lori_sussel/Young_Kim/Sussel_RNASeq_PTPN"
# Path to the count file
count_file <- file.path(base_path,
                     "results/PTPN_KO_countTable_no.txt")
# Path to the sample info
sample_file <- file.path(base_path, "files/sample_info.csv")

# this is the list of genes that interest you. This list I took from 
gene_list <- c("Slc2a3", "Ins1", "Ins2", "Pcsk1", "Abcc8", "Cacna1c",
               "Pdx1", "Mafa", "Nkx6-1", "Gata6", "Gcg", "Sst", "Ghrl",
               "Ppy", "Nkx2-2", "Gata4")
# Load in the files
count_table <- read.table(count_file, sep = "\t", row.names = 1, header = T)
sample_table <- read.table(sample_file, sep = ",", header = T, row.names = 1)
```

## Correlation between samples

First, I look at correlation between samples to determine if there is any structure to the data. Note this correlation is raw reads, so it is just an overview of the data.

```{r corrplot1}
correlation_matrix <- cor(count_table)
corrplot(as.matrix(correlation_matrix), method = "circle", order = "hclust")
```


Everything seems well correlated.

# Make a DESeq2 object

```{r}
# First we need to reorder the counts file so the samples are in the same order as the sample file.
...
# Add a new column to the sample table that is friendlier with R
change_list <- c("Neurog3cre" = "Neurog3cre",
                 "Gata6 fl/fl" = "Gata6" ,
                 "Neurog3cre; RarDN fl/+" = "Neurog3cre_RARdn",
                 "Neurog3cre; Gata6 fl/fl" = "Neurog3cre_Gata6" ,
                 "Neurog3cre; Gata6 fl/fl; RARdn fl/+" =
                   "Neurog3cre_RARdn_Gata6",
                 "Neurog3cre; Gata4 fl/+;  Gata6 fl/fl; RARdn fl/+" =
                   "Neurog3cre_Gata4_Gata6_RARdn",
                 "Neurog3cre; Gata4 fl/+; Gata6 fl/fl; RARdn fl/+" =
                   "Neurog3cre_Gata4_Gata6_RARdn")
Genotype <- sample_table$Genotype
Genotype_R <- change_list[Genotype]
sample_table$Genotype_R <- Genotype_R
# We want to factor with the compare against as the last variable in the factor
sample_table$Genotype_R <- factor(sample_table$Genotype_R,
                                  levels = c("Neurog3cre_RARdn", 
                                             "Neurog3cre_Gata6",
                                             "Neurog3cre_RARdn_Gata6",
                                             "Neurog3cre_Gata4_Gata6_RARdn",
                                             "Gata6",
...
```{r}
# This makes a DESeq2 object where the count data is our count matrix and the colData is our sample data. I am currently making the design based on "genotype" but we can change this if necessary
...
```{r sampledist1}
# This determines distance between samples. It is very simlar to the # correlation done above.
...
Initially this looks pretty promising. For the most part, we see samples clustering by their genotype (although this is not true across the board)

Next we will make a PCA plot to look at distances between samples
```{r pca1}
# Make a PCA plot
pcaData <- plotPCA(vsd, intgroup = "Genotype_R", returnData = T)
percentVar <- round(100 * attr(pcaData, "percentVar"))
ggplot2::ggplot(pcaData, ggplot2::aes(PC1, PC2, color = Genotype_R)) +
...
It looks like the WT stands out here. This is consistent with what I saw in the fastqc, in general the data quality for the WT samples was lower. Another note is that the variance across PC1 is on the lower side.
