...
run_gprofiler <- function(gene_table, pos_name, neg_name,
  ...
  if (!is.null(save_dir)){
    ...
    neg_save_rds <- paste0(neg_name, "_from_", pos_name, "_vs_",
                           neg_name, ".rds")
    saveRDS(pos_gost, file = pos_save_rds)
    saveRDS(neg_gost, file = neg_save_rds)
...
run_gprofiler_all <- function(gene_table, pos_name, neg_name,
  ...
  if (!is.null(save_dir)){
    ...
    saveRDS(gost_res, file = save_rds)
    if(nrow(sig_res) > 1){
...
```{r "Run differential experession chunks"}
...
Do this like he did the samples
